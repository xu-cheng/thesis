...
Next, we present the detailed procedures of the second phase, i.e., authenticating aggregate queries while preserving data confidentiality. Here, we assume that the output from the first phase of candidate object selection has been verified (to be explained in \Cref{sec:aggregate-queries:pgrid}). In what follows, $\{X_1, \dots, X_m\}$ are the feature sets of $m$ candidate objects, and $S = \uplus \{X_i\}$ is the sum set. We first study the \emph{sum}/\emph{count} and \emph{max}/\emph{min}/\emph{FFQ}/\emph{top-$k$} aggregate queries and then extend them to other more advanced aggregate queries.
...
\begin{itemize}
  ...
  \item Deflation checking: $\{(b, 1), (c, 4), (d, 3), (e, 2)\} \cap \{(a, 6)\}\allowbreak = \emptyset$.
\end{itemize}
...
\begin{algorithm}[t]
  ...
  Execute $empty(S-R, R)$ to verify $(S - R) \cap R = \emptyset$\footnotemark[\getrefnumber{fn:aggregate-queries:skip-deflation}]\;%
...
\begin{itemize}
  \item Inflation checking: $\{(a, 6)\}\subseteq \{(a, 6), (b, 1), (c, 4), (d, 3), (e, 2)\}$;
  \item Deflation checking: $\{(b, 1), (c, 4), (d, 3), (e, 2)\} \cap \{(a, 6)\}\allowbreak = \emptyset$;\footnotemark[\getrefnumber{fn:aggregate-queries:skip-deflation}]
