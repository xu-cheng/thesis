...
\section{PA\texorpdfstring{$^2$}{\^{}2}: Privacy-Preserving Authentication Framework for Aggregate Queries}\label{sec:aggregate-queries:pa2}
...
\begin{algorithm}[t]
  ...
  Execute $empty(S-R, R)$ to verify $(S - R) \cap R = \emptyset$\footref{fn:aggregate-queries:skip-deflation}\;%
...
\begin{itemize}
  \item Inflation checking: $\{(a, 6)\}\subseteq \{(a, 6), (b, 1), (c, 4), (d, 3), (e, 2)\}$;
  \item Deflation checking: $\{(b, 1), (c, 4), (d, 3), (e, 2)\} \cap \{(a, 6)\} = \emptyset$;\footref{fn:aggregate-queries:skip-deflation}
...
\subsection{Security of PA\texorpdfstring{$^2$}{\^{}2} Algorithms}
...
\subsection{Accumulating \texorpdfstring{$sum(\cdot)$}{sum(\textcdot)} by Linear Ordering}
...
\savebox{\figbox}{%
  \includegraphics[width=.33\linewidth]{exp-figs/aggregate-queries/pgp_vo.eps}%
}
...
\savebox{\figbox}{%
  \includegraphics[width=.33\linewidth]{exp-figs/aggregate-queries/dimension_vo.eps}%
}
